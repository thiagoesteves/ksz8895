 ...
 *          the messages from ksz8895 gen_server
 */
...
ksz8895_interface_t ksz8895_interface[] =
{
  { "open_ksz8895_driver" , open_ksz8895_driver  },
  { "close_ksz8895_driver", close_ksz8895_driver },
...
#define NUM_KSZ8895_FUN (sizeof(ksz8895_interface)/sizeof(ksz8895_interface_t))

static int process_command(const char *command, char *buf, int *index)
{
  for (int i=0; i<NUM_KSZ8895_FUN; i++)
  {
    if (!strcmp(ksz8895_interface[i].fun_name, command))
    {
      return ksz8895_interface[i].function(buf, index);
    }
  }
